...
exports.register = function(data, callback) {
  ...
  var entity = _.pick(data, 'firstName', 'lastName', 'email', 'password', 'type', 'mobileNumber', 'country');
  async.waterfall([
    function(cb) {
    ...
    function(user, cb) {
      var message = {verifyAccountTokenExpiry: user.verifyAccountTokenExpiry, link: config.VERIFY_ACCOUNT_LINK.replace(':token', user.verifyAccountToken)};
      queues.publish(queueNames.registeruser, message, cb);
    }
  ], callback);
};
...
exports.forgotPassword = function(email, callback) {
  async.waterfall([
    ...
    function(user, cb) {
      var message = {resetPasswordTokenExpiry: user.resetPasswordTokenExpiry, link: config.RESET_PASSWORD_LINK.replace(':token', user.resetPasswordToken)};
      queues.publish(queueNames.forgotpassword, message, cb);
    }
  ], callback);
};
